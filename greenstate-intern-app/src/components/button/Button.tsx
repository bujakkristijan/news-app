...
import { TypographyButtonSize, TypographyFontSize, TypographyLineHeight } from '../../shared/theme/theme';
...
export const Button = ({ size = "sm", children, isDisabled = false }: ButtonProps) => {
    const { fontSize, lineHeight } = getFontDetails(size);

    return (
        <StyledButton size={size} disabled={isDisabled}>
            <Text fontSize={fontSize} fontWeight="extraBold" lineHeight={lineHeight} fontFamily="inter" color={isDisabled ? "darkGrey" : "white"}>
                {children}
            </Text>
        </StyledButton>
    );
